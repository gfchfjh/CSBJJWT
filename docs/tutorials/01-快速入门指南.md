...
> **目标**: 快速完成安装和基础配置，成功转发第一条消息
> 
> **版本**: v15.0.0 傻瓜式应用版（傻瓜式应用版（深度优化完成））
> **预计时间**: 5分钟
> **新功能**: AI智能映射 | Chrome扩展自动导入 | 用户友好错误提示 | 内置帮助系统
...
- ✅ 一键安装程序（Windows .exe / macOS .dmg / Linux .AppImage）
- ✅ 首次启动自动打开配置向导
- ✅ AI智能推荐频道映射
- ✅ Chrome扩展自动Cookie导入
...
1. **下载安装包**
   ...
   根据您的操作系统选择：
   - Windows: `KOOK-Forwarder-Setup-v15.0.0.exe` (~150MB)
   - macOS Intel: `KOOK-Forwarder-v15.0.0-macOS-x64.dmg` (~160MB)
   - macOS Apple Silicon: `KOOK-Forwarder-v15.0.0-macOS-arm64.dmg` (~160MB)
   - Linux: `KOOK-Forwarder-v15.0.0-Linux-x64.AppImage` (~155MB)

2. **解压到任意目录**
   
   ```bash
   # 解压到任意目录（中文路径也可以）
   unzip KOOK-Forwarder-v15.0.0-*.zip
   cd KOOK-Forwarder
   ```
...
2. **AI生成智能推荐**
   ...
   ```
   公告频道 到 Discord: #announcements (95%) ✅
   闲聊      到 Discord: #general (92%) ✅
   游戏讨论  到 Discord: #gaming (90%) ✅
   技术交流  到 Discord: #tech (85%) ✅
   ...
   ```
