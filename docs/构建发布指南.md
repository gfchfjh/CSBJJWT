...
**💡 提示**：如果是第一次构建，强烈建议阅读 [LOCAL_BUILD_GUIDE.md](../LOCAL_BUILD_GUIDE.md)，包含详细的11步Windows构建、10步macOS构建、10步Linux构建指南。
...
# 3. 创建Release tag并推送
./scripts/create-release.sh v1.14.0
...
docker tag kook-forwarder:latest ghcr.io/gfchfjh/csbjjwt:v1.14.0
```
...
docker push ghcr.io/gfchfjh/csbjjwt:v1.14.0
...
```bash
# 1. 基于发布tag创建分支
git checkout -b hotfix/v1.14.0 v1.14.0
...
# 5. 推送并发布
git push origin hotfix/v1.14.0 v1.13.4
```
