...
class HealthChecker:
    ...
    def __init__(self):
        ...
        self.check_interval = getattr(settings, 'health_check_interval', 300)  # 默认5分钟
        self.last_results = {}
        self.listeners = []  # 状态变更监听器
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        return {
                            "status": "healthy",
                            "message": "Discord Webhook可用"
                        }
                    else:
                        return {
                            "status": "unhealthy",
                            "message": f"Discord Webhook返回 {response.status}"
                        }
        except asyncio.TimeoutError:
            return {
                "status": "unhealthy",
                "message": "Discord Webhook连接超时"
            }
        except Exception as e:
            return {
                "status": "unhealthy",
                "message": f"Discord Webhook检查失败: {str(e)}"
            }
    ...
    async def check_telegram_bot(self, token: str) -> Dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if data.get("ok"):
                        return {
                            "status": "healthy",
                            "message": f"Telegram Bot可用: @{data['result']['username']}"
                        }
                    else:
                        return {
                            "status": "unhealthy",
            ...
            return {
                "status": "unhealthy",
                "message": "Telegram API连接超时"
            }
        except Exception as e:
            return {
                "status": "unhealthy",
                "message": f"Telegram Bot检查失败: {str(e)}"
            }
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if data.get("code") == 0:
                        ...
                        return {
                            "status": "unhealthy",
                            "message": f"飞书应用认证失败: {data.get('msg')}"
                        }
        except asyncio.TimeoutError:
            return {
                "status": "unhealthy",
    ...
    async def check_all_bots(self) -> List[Dict[str, Any]]:
        ...
        for bot in bot_configs:
            ...
            if platform == 'discord':
                ...
                result = await self.check_feishu_app(
                    config.get('app_id'),
    ...
    async def check_scrapers(self) -> Dict[str, Any]:
        ...
        active_scrapers = sum(
            1 for scraper in scraper_manager.scrapers.values()
            if scraper.is_running
        )
        
        return {
            "total": total_scrapers,
            "active": active_scrapers,
            "status": "healthy" if active_scrapers > 0 else "warning",
            "message": f"{active_scrapers}/{total_scrapers} 个抓取器运行中"
        }
    ...
    async def check_storage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "message": f"磁盘使用: {round(usage_percent, 1)}%"
            }
        except Exception as e:
            return {
                "status": "error",
