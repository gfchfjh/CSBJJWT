...
from .message_parser import message_parser  # ✨ P0-1优化: 新增完整消息解析器
...
class KookScraper:
    ...
    async def _process_websocket_message(self, payload):
        """处理WebSocket消息（✨ P0-1优化: 使用完整消息解析器）"""
        try:
            ...
            if data.get('type') == 'MESSAGE_CREATE':
                ...
                message = {
                    ...
                    'server_id': parsed['guild_id'],
                    'content': parsed['content'],
                    'message_type': parsed['type'],
                    'sender_id': parsed['author'].get('id'),
                    'sender_name': parsed['author'].get('username'),
                    'sender_avatar': parsed['author'].get('avatar'),
                    ...
                    'quote': {
                        'message_id': parsed['quote'].quoted_message_id,
                        'author': parsed['quote'].quoted_author,
                        'content': parsed['quote'].quoted_content
                    } if parsed['quote'] else None,
                    'link_preview': {
                        'url': parsed['link_preview'].url,
                        'title': parsed['link_preview'].title,
                        'description': parsed['link_preview'].description,
                        'image': parsed['link_preview'].image
                    } if parsed['link_preview'] else None,
                    'attachment': {
                        'filename': parsed['attachment'].filename,
                        'size': parsed['attachment'].size,
                        'url': parsed['attachment'].url,
                        'type': parsed['attachment'].file_type
                    } if parsed['attachment'] else None,
                    'mentions': parsed['mentions'],
                    'mention_all': any(m['type'] == 'all' for m in parsed['mentions']),
                    
                    'cookies': cookies_dict,  # 传递Cookie用于下载防盗链资源
                }
                
                logger.info(f"✨ 收到新消息: {message['message_id']} (类型: {message['message_type']}, "
                           f"图片: {len(message['image_urls'])}, 反应: {len(message['reactions'])}, "
                           f"引用: {'是' if message['quote'] else '否'})")
    ...
    async def _reconnect(self):
        ...
        try:
            # ✨ P0-1优化: 实现指数退避
            # 等待时间: 30秒, 60秒, 120秒, 240秒, 300秒(最大)
            wait_time = min(30 * (2 ** (self.reconnect_count - 1)), 300)
            
            logger.info(f"⏰ 等待 {wait_time} 秒后尝试重新连接...")
            ...
            if await self._check_login_status():
                ...
                # ✨ P0-1新增: 发送重连成功通知
                await self._send_reconnect_success_notification()
            else:
    ...
    async def _send_reconnect_success_notification(self):
        """发送重连成功通知（桌面通知+WebSocket）"""
        try:
            ...
            logger.info("✅ 已发送重连成功通知")
        except Exception as e:
            logger.error(f"发送重连成功通知失败: {e}")
    
    async def _send_disconnect_alert(self):
        """发送断线告警（桌面通知+邮件）"""
        try:
            from ..api.websocket import websocket_manager
            ...
            await websocket_manager.broadcast({
                ...
                "data": {
                    "account_id": self.account_id,
                    "message": f"账号已断线，达到最大重连次数({self.max_reconnect})",
                    "severity": "error",
                    "timestamp": asyncio.get_event_loop().time()
                }
            })
            ...
            if settings.email_alert_enabled:
                account_email = db.get_account_by_id(self.account_id).get('email', '未知')
                await email_sender.send_alert(
                    subject=f"[KOOK转发系统] 账号断线告警",
                    body=f"""
                    KOOK账号已断线：
                    
                    账号: {account_email}
                    账号ID: {self.account_id}
                    重连次数: {self.reconnect_count}
                    最大重连次数: {self.max_reconnect}
            ...
            logger.error("⚠️ 已发送断线告警")
        except Exception as e:
            logger.error(f"发送断线告警失败: {e}")
